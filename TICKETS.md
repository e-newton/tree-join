...
**Convention:** every ticket that adds production logic also adds unit tests for that logic in the same ticket. Test *infrastructure* gets its own ticket (T-03); test *coverage* is not deferred.
...
- `object_type` uses `{` `}`, comma *or* semicolon separator. Detect existing separator in the node; preserve it on split, default to `;` if the node is empty.
- Fixtures.
...
Linear: T-01 → T-02 → T-03 → T-04 → T-05 → T-06 → T-07 → T-08 → T-09 *(first usable extension)* → T-10 → T-11 → T-12 → T-13 → T-14 → T-15 → T-16 → T-17 → T-18.
